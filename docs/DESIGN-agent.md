...
**状態**: **実装済み**（`harness/agent.py`、AGENT_VERSION=local-agent-v2）。
...
### ツール（5つ。すべて作業ディレクトリ内に限定）

| ツール | 引数 | 動作 |
|---|---|---|
| `list_files` | — | 作業ディレクトリのファイル一覧（隠しテストは除外） |
| `read_file` | path | 内容を行番号付きで返す（バグ/脆弱の行番号回答に必要） |
